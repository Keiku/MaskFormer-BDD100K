...
BDD100K_DRIVABLE_AREA_CATEGORIES = [
...
def _get_bdd100k_drivable_area_meta():
    # Id 0 is reserved for ignore_label, we change ignore_label for 0
    # to 255 in our pre-processing, so all ids are shifted by 1.
    stuff_ids = [k["id"] for k in BDD100K_DRIVABLE_AREA_CATEGORIES]
    assert len(stuff_ids) == 3, len(stuff_ids)

    # For semantic segmentation, this mapping maps from contiguous stuff id
    # (in [0, 91], used in models) to ids in the dataset (used for processing results)
    stuff_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(stuff_ids)}
    stuff_classes = [k["name"] for k in BDD100K_DRIVABLE_AREA_CATEGORIES]
...
def register_bdd100k_drivable_area(root):
    ...
    for name, dirname in [("train", "train"), ("val", "val")]:
        ...
        name = f"bdd100k_drivable_area_{name}"
...
register_bdd100k_drivable_area(_root)
